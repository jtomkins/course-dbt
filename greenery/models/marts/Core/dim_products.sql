...
SELECT
  ...
  ,int_total_products_sold.total_number_sold
  FROM {{ ref('stg_greenery__products') }} 
    Left join {{ ref('int_total_products_sold')}}
    on stg_greenery__products.product_guid = int_total_products_sold.product_guid 
